...
def novosort_merge_se(config_file,sample_list,wait):
    ...
    for sample in fh:
        ...
        if n > 1:
            novosort_merge_se_cmd=novosort + " --threads " + threads + " --ram " + ram + "G --assumesorted --output " + sample + '.merged.bam --index --tmpdir ./TMP ' + bam_string
            sys.stderr.write(date_time() + novosort_merge_se_cmd + "\n")
            try:
                subprocess.check_output(novosort_merge_se_cmd,shell=True) 
...
if __name__ == "__main__":
    ...
    novosort_merge_se(config_file,sample_list,wait)
